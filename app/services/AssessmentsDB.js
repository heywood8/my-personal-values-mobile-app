...
export async function getRankedResults(assessmentId) {
  ...
  return rows.map((row) => ({
    ...
    // No `archived` here, deliberately. A record is a record — a value archived
    // after this run still belongs to it — and the one surface that does need to
    // know, the alignment wheel, asks the catalogue rather than this snapshot,
    // because only the catalogue is current in both directions.
    score: row.score,
    normalized: row.normalized,
  }));
}
